/* ***************************************************************************** */
/* Mika's Find a mole typing trainer  Javascript source code                     */
/*                                                    Ver2.05.01 2024/11/5       */
...
MIKA_mes0="(^_^) Mika's Find a Mole Typing Trainer (^_^)/~~ Ver2.05.01";
MIKA_mes0a="(^_^) Mika's Find a Mole Beginner (^_^)/~~";
MIKA_mes0b="(^_^) Mika's Find a Mole Standard (^_^)/~~";
MIKA_mes0c="(^_^) Mika's Find a Mole Expert (^_^)/~~";
MIKA_mesta="(^_^) Find a Mole Beginner (%s) (^_^)/~~";
MIKA_mestb="(^_^) Find a Mole Standard (%s) (^_^)/~~";
MIKA_mestc="(^_^) Find a Mole Expert (%s) (^_^)/~~";
MIKA_mest2a="Beginner           Typing record second         Date        Practice time";
MIKA_mest2b="Standard           Typing record second         Date        Practice time";
MIKA_mest2c="Expert             Typing record second         Date        Practice time";
MIKA_mesi1="Press Enter key to try again";
MIKA_mesi2="Press ESC key to retrun to Menu";
MIKA_mesi3="＼(^O^)／ Congratulations You have updated your record ＼(^O^)／";
MIKA_abort_mes="Press ESC key to cancel";
MIKA_return_mes="Press ESC key to retrun to Menu";
MIKA_key_type_mes="．．";
MIKA_menu_mes_s=[ /* 初期メニュー メニュー項目 */
		"Find a Mole Beginner",
		"Find a Mole Standard",
		"Find a Mole Expert",
...
window.onload = function() {
 const MIKAMOLE = document.getElementById("MIKAMOLE"); /* MIKAMOLE キャンバス取得 */
    if (MIKAMOLE.getContext) { /* MIKAMOLE キャンバスが存在した場合 */
      const g = MIKAMOLE.getContext("2d");//2次元描画
		MIKA_win_size_width=MIKAMOLE.width; /* 表示ウィンドー横サイズ取得 */
		MIKA_win_size_height=MIKAMOLE.height; /* 表示ウィンド縦サイズ取得 */
		readcookie(); /* クッキー読み込み */
...
{
	var nChar;
	const MIKAMOLE = document.getElementById("MIKAMOLE"); /* MIKAMUGU キャンバス取得 */
 	if (MIKAMOLE.getContext) { /* MIKAMOLEのキャンバスが存在した場合 */
    	const g = MIKAMOLE.getContext("2d"); /* 2次元描画 */
...
function dispmaxspeed(g,flag) /* 初級 中級 上級 の最高入力速度と 達成日を表示 */
	{
			...
			a="Min Record"+formatd(MIKA_type_speed_record[MIKA_type_kind_no],4)+"sec";/* 最高速度メッセージ作成 */
			cslput(g,1*16,1,a); /* 最高速度メッセージ表示 */
			b="Date"+MIKA_type_date_record[MIKA_type_kind_no]; /* 達成日メッセージ作成 */
			cslput(g,1*16,18*8,b); /* 達成日メッセージ表示 */
	}
...
function dispupmes(g,flag) /* タイプ速度を更新したときのメッセージを表示 */
	{
		...
		cslput(g,20*16,8*8,MIKA_mesi3); /* 指定位置に「おめでとう、記録を更新しました」のメッセージを表示 */
	}
...
function convkookie() /* cookie の連想配列データーを成績に変換して格納 */
{
	...
	var i,ii;
	var a;
	if('MIKAMOLE' in MIKA_cookie_array1) /* 連想配列に 'MIKAMOLE'があった場合 */
	{
		a=MIKA_cookie_array1['MIKAMOLE']
		ii=0;
